...
### Gap 1: No Parallel Execution

Batch commands process items sequentially. No built-in parallelism.

**Impact:** Slow for large backlogs.

**Fix:** Add `--parallel <n>` flag for concurrent processing.
...
**Fix:** Track current item in index or separate state file.
...
**Fix:** Return error on ambiguous match instead of first match.
